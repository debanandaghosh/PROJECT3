...
var bot = new builder.UniversalBot(connector, (session, args, next) => {
    ...
    tableSvc.createTableIfNotExists('', function (error, result, response) {
        if (!error) {
            // Table exists or created
        }
        else { session.send(error.message);}
    });
var entGen = azure.TableUtilities.entityGenerator;
    var task = {
        PartitionKey: entGen.String(session.message.text),
        RowKey: entGen.String('1'),
        question: entGen.String(session.message.text)
    };
    tableSvc.insertEntity('smrtnovtable', task, function (error, result, response) {
    })
